...
def evaluate_root(tree: SearchTree,
    ...
    Args:
        tree (SearchTree): SearchTree 对象，表示蒙特卡洛搜索树。
        game (SearchableEnv): SearchableEnv 对象，表示一个可以被蒙特卡洛搜索处理的游戏环境。
        net (Network): Network 对象，用于预测状态价值、走子概率和策略热度。
        rng (RandomState): RandomState 对象，表示一个随机数生成器。
