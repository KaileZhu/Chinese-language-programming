...
@dataclass
class HexGameState:
    color: int  # 游戏进行时的玩家，0 代表红色方，1 代表蓝色方
    legal_moves: np.ndarray  # 当前可行的落子位置，即无棋子的格子位置
    result: int  # 当前游戏的结果，0 表示未结束，1 表示红色方胜利，2 表示蓝色方胜利，3 表示平局
    board: np.ndarray  # 游戏棋盘
...
class HexGame:
    ...
    def __init__(self, board_size: int = 11) -> None:
        ...
        self.impl = HexGameImpl(self.board_size)  # 游戏实现类
        self._game_snapshot = None  # 游戏快照，用于回撤
        self.reset()  # 重置游戏

    def __getstate__(self):
        """Pickling support"""
        # pickle 序列化时调用此方法获取当前状态
    ...
    def reset(self):
        self.impl = HexGameImpl(self.board_size)  # 重置游戏状态
